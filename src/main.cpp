...
#if defined(NDEBUG)
# error "Realcoin cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0xc00ab082d74527c1bff82f349adcf9610b6b652f0932ab600295bdb51cdc3748");
...
{
    // Realcoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
{
    int64 nSubsidy = 20.9 * COIN;

    // Subsidy is cut in half every 840000 blocks, which will occur approximately every 4 years
    nSubsidy >>= (nHeight / 840000); // Realcoin: 840k blocks in ~4 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 1 * 24 * 60 * 60; // Realcoin: 3.5 days
static const int64 nTargetSpacing = 4 * 60; // Realcoin: 2.5 minutes
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Realcoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0x80daf2c47fb3e6abe1cc07c4805aced26bdc25ba3d5b79c39335d1c754b2dc5c");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("0447868046d0128c4da84afd4fa3406e6d267d12c636fa2f82d16a6c8c2b55d00aa7a64e59bf99e9ab7579039bd14b27efd505e355219138cbd711e78b68860a1c") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1588769146;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 2084557549;

        if (fTestNet)
        {
            block.nTime    = 1588769122;
            block.nNonce   = 385378629;
        }
...
if (false && block.GetHash() != hashGenesisBlock)
        ...
        assert(block.hashMerkleRoot == uint256("0x1d14811fd8f31fcceef2e711c139d0c82cf7c4f5a56b7122bff37a995310b10e"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xfa, 0xc3, 0xba, 0xd3 }; // Realcoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("RealcoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("RealcoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static RealcoinMiner(CWallet *pwallet)
{
    printf("RealcoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("realcoin-miner");
    ...
    try { loop {
        ...
        printf("Running RealcoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("RealcoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&RealcoinMiner, pwallet));
}
